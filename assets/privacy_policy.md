...
| Android 无障碍服务（仅截屏） | 您使用系统下拉面板的「快捷截图分析磁贴」时 | 仅截屏事件触发的屏幕画面（设备本地处理，**不上传**） |

DeepSeek 调用使用您自己填写的 API Key（`Authorization: Bearer`），请求体仅包含您本次的输入与上文提示词，**不包含**您的设备信息、API Key 本身或账号标识。请求走 HTTPS。

关于「快捷截图分析磁贴」的触发时机与数据流：

- 仅在您主动点击系统下拉面板的「Nous记事」磁贴时触发，不会自动运行或定时运行
- 截屏画面通过本机 ML Kit（`google_mlkit_text_recognition`）完成文字识别，**不联网**；识别后的文字将作为输入交给您在编辑器中触发的「AI 一键调整」流程，是否上传到 DeepSeek 由您当时的选择决定
- 截屏临时缓存于应用沙盒内，分析完成后立即清除；不写入任何持久化文件
- 您可随时在「系统设置 → 无障碍 → 已安装的应用」中关闭本应用的无障碍权限，关闭后磁贴将无法截屏
...
| 精确闹钟 | 定时发送通知 | 否 |
| 日历读写 | "加入日历"功能 | 否 |
| 无障碍服务（Android） | 「快捷截图分析磁贴」一键截屏 | 否（关闭后无法使用磁贴截屏，可改用编辑器内「AI 截图」按钮） |
...
| Android Accessibility Service (screenshot only) | You tap the "Nous Mind" tile in the system quick-settings panel | The captured screen image, processed on-device only (not uploaded) |
...
About the "Quick-settings screenshot tile" trigger and data flow:

- Triggers only when you actively tap the "Nous Mind" tile in the system quick-settings panel; never runs automatically or on a schedule
- The captured screen image is processed on-device by ML Kit (`google_mlkit_text_recognition`); the OCR text is **not** sent to any network. It is handed to the "AI adjust" flow you trigger inside the editor, and whether it is sent to DeepSeek depends on your choice at that moment
- The screenshot is temporarily cached in the app sandbox and cleared immediately after analysis; nothing is written to any persistent file
- You can revoke the accessibility permission at any time in "System Settings → Accessibility → Installed services"; the tile will then be unable to capture screens
...
| Android Accessibility Service | "Quick-settings screenshot tile" one-tap capture | No (the tile is disabled if you deny it; you can still use the "AI screenshot" button inside the editor) |
