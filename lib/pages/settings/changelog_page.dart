...
const List<_ChangelogVersion> _changelog = <_ChangelogVersion>[
  _ChangelogVersion(
    version: '1.7.1',
    notes: <String>[
      '新增「快捷截图分析磁贴」：系统下拉面板新增「Nous 记事」磁贴，一键截屏后通过本地 ML Kit OCR 识别文字，并直接跳转到「AI 一键调整为提醒」流程；无障碍截图全程不弹系统截屏音，磁贴触发后展示美化过的延迟确认弹窗，避免误触。',
      '通知后台操作可落库：在 App 未启动时点击「稍后提醒」或「标记完成」会真正写入数据库 ——「稍后提醒」按设置中的「稍后时长」更新提醒时间并重新调度通知，「标记完成」会软删除提醒并取消通知；不再出现点了没反应的孤立操作。',
      '回到 App 自动同步：进入前台时自动从数据库刷新提醒列表与回收站数量，并按当前时间重排所有系统通知计时器；解决「刚开机的提醒在重启后不再响」「时区切换后时间错位」等问题。',
      '修复热更新图片缓存、设置页侧滑误返回桌面、标签弹窗键盘溢出等若干问题。',
    ],
  ),
  _ChangelogVersion(
    version: '1.7.0',
    notes: <String>[
      '新增「新手教程」：在「关于」页面中提供清晰易懂的图文使用指南，快速掌握提醒、标签、灵感与 AI 智能助手功能。',
      '新增「标签系统」：为提醒引入「工作 / 生活 / 学习 / 娱乐」四大内置标签（不可删改），并在「设置 → 标签」中支持创建最多 10 个自定义标签（提供 15 色调色板，可重命名/改色/删除）；首页新增筛选图标可按标签过滤提醒，提醒编辑页可一键打标或在弹层内快速新建标签，「已完成」状态现已统一为系统标签、AI 助手也能在调整/识别时自动为提醒分配合适的标签。',
      '新增「灵感 AI 助手」：灵感列表支持多选后批量交给 AI 分析，可按需勾选「总结想法 / 一键加到提醒 / 提取待办清单 / 提取核心主题」四项能力，提示词同样在「设置 → AI 助手 → 提示词」中可自定义并支持预览。',
      '灵感支持「软删除（回收站）」：删除的灵感现在会移入回收站并保留 30 天，防止误删，支持随时单条/批量恢复与永久清理。',
      '灵感列表新增日期范围筛选：顶部日历图标可按时间段过滤，清除后回到全部。',
      '回收站全新改版：升级为「提醒事项」与「灵感」双标签页，各自独立进行全部恢复与永久删除，并支持直接点击卡片浏览回收站内容。',
      '自定义提示词按钮与顺序优化：重塑保存、预览和恢复默认按钮样式，极大地优化了屏幕横向空间以解决溢出 Bug，并调整为更直观的按钮优先级顺序。',
      '优化数据页面下的回收站数量统计显示，并在 foreground 唤醒时自动扫描并清理过期提醒和灵感。',
    ],
  ),
  _ChangelogVersion(
    version: '1.6.0',
    notes: <String>[
      '本版本对 AI 分析器、启动入口、提醒视图模型三个核心模块做了底层拆分重构，代码总量减少约 33%；同步修复了备份导出 / AI 错误面板等场景下的潜在空指针异常，以及冷启动通知路由的边界处理。',
      '用户协议新增「用户行为规范」「内容归属」「未成年人保护」等 6 项条款，开发者邮箱已更新为 dsdogs@outlook.com',
      'AI 助手首次启用时需先阅读并同意《用户协议》和《隐私政策》，后续启用不再重复提醒',
      '应用图标焕新',
    ],
  ),
  _ChangelogVersion(
